...
async function doLogin() {
  ...
  // 2. Verify against admin_users table — match by auth user id when available,
  //    fall back to email. Surface the real reason on failure.
  const userId = authData?.user?.id;
  let adminQuery = supabase.from('admin_users').select('*');
  // Prefer id match if the table has an `id` column tied to auth.users; email is the safe fallback.
...
async function checkSession() {
  ...
  if (adminErr || !adminData || adminData.is_active === false) {
    // Session is valid with Supabase but admin gate doesn't pass — clear it
    // so the user sees the login screen rather than a half-broken state.
    await supabase.auth.signOut();
    return;
  }
...
function initAdmin() {
  ...
  // Delegated handlers for the orders table — safer than inline onclick="…"
  // because data-* values are never re-parsed as JS.
  ...
  tbody.addEventListener('click', (e) => {
    ...
    else if (action === 'copylink') window.copyLink(url);
  });
  ...
  // Auto refresh
  refreshTimer = setInterval(() => loadOrders(true), CONFIG.AUTO_REFRESH_MS);
...
async function loadOrders(silent = false) {
  ...
  // Note on search: Supabase .or() across joined tables is limited, so we
  // page the server-side filtered set and refine matches client-side below.
  // For larger datasets, add a tsvector column or a search view server-side.
  ...
  // If another loadOrders started after this one, discard our result.
  ...
  // Client-side search refinement (for cross-table search)
  if (search) {
  ...
  // Client-side dzongkhag filter (if not already filtered server-side)
  if (dzong) {
...
async function updateStats() {
  // Try the RPC first for accurate, server-side totals.
  ...
  // RPC missing or errored — fall back to a lightweight client-side calc so
  // the skeletons don't sit forever. Note: this only counts the current page
  // of orders that's already in memory, so totals reflect the page, not the
  // whole DB. Deploy the get_order_stats RPC for accurate site-wide numbers.
...
function renderOrders() {
  ...
  orders.forEach(order => {
    ...
    // Note: data-* attribute values are decoded by the browser AFTER HTML
    // parsing and exposed via element.dataset as raw strings — they are NOT
    // re-interpreted as JS. This makes them safe for arbitrary user content
    // (e.g. product URLs with apostrophes) where inline onclick="..." was not.
    ...
    tr.innerHTML = `
      ...
      <td>
        <div class="actions">
          ...
          <button class="btn-icon btn-link" title="Copy Product Link" data-action="copylink" data-url="${escapeHtml(firstItem.product_link || '')}">🔗</button>
        </div>
      </td>
    `;
    frag.appendChild(tr);
...
// Keep the header checkbox in sync with row selections (was previously
// stuck unchecked even when every visible row was selected).
...
window.exportCSV = async function() {
  ...
  // Mirror the active filters so the export matches what the admin sees.
  const search = document.getElementById('filterSearch').value.trim();
  const status = document.getElementById('filterStatus').value;
  const dzong  = document.getElementById('filterDzongkhag').value;
  const trip   = document.getElementById('filterDate').value;
  ...
  // Apply the same client-side refinements as the table view.
  let rowsData = data;
  if (search) {
    const s = search.toLowerCase();
  ...
  // Prefix values that begin with =,+,-,@ with a single quote so Excel
  // doesn't interpret them as formulas (CSV injection mitigation).
  const csvEscape = (v) => {
  ...
  // CRLF line endings are the safest for Excel/Windows.
